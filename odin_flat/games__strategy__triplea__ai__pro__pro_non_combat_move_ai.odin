...
pro_non_combat_move_ai_move_units_to_defend_territories :: proc(
	...
	for {
		...
		// Land/air units are not currently appended at insertion time
		// (plumbing reproducibly trips a separate runtime bug); rebuild
		// the missing entries from `my_unit_territories × territory.units`
		// so per-unit defend sets are still computed for non-naval units
		...
		_defend_opts_order_use := _defend_opts_order
		{
			...
			defer delete(seen)
			for u in _defend_opts_order { seen[u] = {} }
			appended_extra := false
			for u in _defend_opts_order { append(&_defend_opts_order_fallback, u) }
			...
			if appended_extra || len(_defend_opts_order) == 0 {
