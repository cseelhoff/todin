...
deserialize_game_data :: proc(root: json.Object) -> ^Game_Data {
	gd := new(Game_Data)
	...
	gd.state = game_data_state_new(gd)
	...
	// Resources
	gd.resource_list = resource_list_new(gd)
	if res_arr, ok := get_array(root, "resources"); ok {
		for item in res_arr {
			if name, name_ok := item.(json.String); name_ok {
				r := resource_new_simple(strings.clone(name), gd)
				resource_list_add_resource(gd.resource_list, r)
			}
		}
	}

	// Unit types
	gd.unit_type_list = new(Unit_Type_List)
	gd.unit_type_list.unit_types = make(map[string]^Unit_Type)
	...
	// Players
	gd.player_list = new(Player_List)
	gd.player_list.players = make(map[string]^Game_Player)
	...
	gd.player_list.null_player = player_list_create_null_player(gd)
	...
	// Territories
	gd.game_map = new(Game_Map)
	gd.game_map.territory_lookup = make(map[string]^Territory)
	gd.game_map.connections = make(map[^Territory]map[^Territory]struct{})
	if terr_arr, ok := get_array(root, "territories"); ok {
		for item in terr_arr {
			if t_obj, t_ok := item.(json.Object); t_ok {
				...
				if t != nil {
					...
					// "No neighbors" panic.
					gd.game_map.connections[t] = make(map[^Territory]struct{})
				}
			}
		}
		...
		for item in terr_arr {
			...
			for n in n_arr {
				...
				game_map_add_connection(gd.game_map, t, other)
			}
		}
	}
	...
	// (Disabled in this in-package loader: the clone path's serializer
	// already emits territoryAttachment data so the sidecar overlay
	// would either be missing or stale.)
	// apply_xml_territory_attachments(gd)

	// Units
	gd.units_list = new(Units_List)
	gd.units_list.units = make(map[Uuid]^Unit)
	...
	// Alliances
	gd.alliances = new(Alliance_Tracker)
	gd.alliances.alliances = make(map[string][dynamic]^Game_Player)
	if alliances_obj, ok := get_object(root, "alliances"); ok {
		for alliance_name, val in alliances_obj {
			if players_arr, arr_ok := val.(json.Array); arr_ok {
				player_list: [dynamic]^Game_Player
	...
	// Properties
	gd.properties = new(Game_Properties)
	gd.properties.constant_properties = make(map[string]Property_Value)
	gd.properties.editable_properties = make(map[string]^Editable_Property)
	...
	// Relationships
	gd.relationships = new(Relationship_Tracker)
	gd.relationships.relationships = make(map[Related_Players]^Relationship)

	// Relationship types
	gd.relationship_type_list = new(Relationship_Type_List)
	gd.relationship_type_list.relationship_types = make(map[string]^Relationship_Type)
	if rt_arr, ok := get_array(root, "relationshipTypes"); ok {
		for item in rt_arr {
			if rt_obj, rt_ok := item.(json.Object); rt_ok {
				name := get_string(rt_obj, "name")
				rt := new(Relationship_Type)
				...
				rta := new(Relationship_Type_Attachment)
				switch name {
				case "default_war_relation", "war":
					rta.arche_type = RELATIONSHIP_TYPE_ATTACHMENT_ARCHETYPE_WAR
				case "default_allied_relation", "allied", "self_relation":
					rta.arche_type = RELATIONSHIP_TYPE_ATTACHMENT_ARCHETYPE_ALLIED
				case:
					rta.arche_type = RELATIONSHIP_TYPE_ATTACHMENT_ARCHETYPE_NEUTRAL
				}
				rta.can_move_land_units_over_owned_land = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.can_move_air_units_over_owned_land = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.alliances_can_chain_together = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.is_default_war_position = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.upkeep_cost = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.can_land_air_units_on_owned_land = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.can_take_over_owned_territory = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.gives_back_original_territories = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.can_move_into_during_combat_move = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.can_move_through_canals = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rta.rockets_can_fly_over = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
				rt.attachments = make(map[string]^I_Attachment)
				rt.attachments["relationshipTypeAttachment"] = cast(^I_Attachment)rta
	...
	if _, has_null := gd.relationship_type_list.relationship_types["null_relation"]; !has_null {
		nr := new(Relationship_Type)
		nr.named.base.name = strings.clone("null_relation")
		nr.attachments = make(map[string]^I_Attachment)
		nrta := new(Relationship_Type_Attachment)
		nrta.arche_type = RELATIONSHIP_TYPE_ATTACHMENT_ARCHETYPE_WAR
		nrta.can_move_land_units_over_owned_land = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.can_move_air_units_over_owned_land = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.alliances_can_chain_together = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.is_default_war_position = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.upkeep_cost = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.can_land_air_units_on_owned_land = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.can_take_over_owned_territory = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.gives_back_original_territories = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.can_move_into_during_combat_move = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.can_move_through_canals = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nrta.rockets_can_fly_over = RELATIONSHIP_TYPE_ATTACHMENT_PROPERTY_DEFAULT
		nr.attachments["relationshipTypeAttachment"] = cast(^I_Attachment)nrta
		...
		for item in rel_arr {
			if rel_obj, rel_ok := item.(json.Object); rel_ok {
				...
				if p1_found && p2_found && rt_found {
					...
					key1 := make_Relationship_Tracker_Related_Players(p1, p2)
					key2 := make_Relationship_Tracker_Related_Players(p2, p1)
	...
	relationship_tracker_set_self_relations(gd.relationships)
	relationship_tracker_set_null_player_relations(gd.relationships)
				...
				if costs_obj, cok := get_object(pr_obj, "costs"); cok {
					for res_name, val in costs_obj {
						...
						if r, found := gd.resource_list.resources[res_name]; found {
							production_rule_add_cost(pr, r, amount)
						}
					}
				}
				if results_obj, rok := get_object(pr_obj, "results"); rok {
					for key_name, val in results_obj {
						...
						if ut, found := gd.unit_type_list.unit_types[key_name]; found {
							integer_map_put(&pr.results, rawptr(ut), amount)
						} else if r, found := gd.resource_list.resources[key_name]; found {
							integer_map_put(&pr.results, rawptr(r), amount)
						}
					}
				}
				production_rule_list_add_production_rule(gd.production_rule_list, pr)
			}
		}
	}

	// Production frontiers
	gd.production_frontier_list = production_frontier_list_new(gd)
	if pf_arr, ok := get_array(root, "productionFrontiers"); ok {
		for item in pf_arr {
			if pf_obj, pf_ok := item.(json.Object); pf_ok {
				...
				pf := production_frontier_new(frontier_name, gd)
				if rules_arr, rok := get_array(pf_obj, "rules"); rok {
					for r_val in rules_arr {
						if rule_name, rn_ok := r_val.(json.String); rn_ok {
							if rule := production_rule_list_get_production_rule(gd.production_rule_list, rule_name); rule != nil {
								append(&pf.rules, rule)
							}
						}
					}
				}
				production_frontier_list_add_production_frontier(gd.production_frontier_list, pf)
			}
		}
	}
	...
	if pl_arr2, ok := get_array(root, "players"); ok {
		for item in pl_arr2 {
			...
			if pf_name := get_string(p_obj, "productionFrontier"); pf_name != "" {
				if pf := production_frontier_list_get_production_frontier(gd.production_frontier_list, pf_name); pf != nil {
	...
	if tf_obj, ok := get_object(root, "technologyFrontier"); ok {
		...
		gd.technology_frontier = technology_frontier_new(tf_name, gd)
		if techs_arr, tok := get_array(tf_obj, "techs"); tok {
			...
			by_display: map[string]^Tech_Advance
			by_display = make(map[string]^Tech_Advance)
...
deserialize_sequence :: proc(obj: json.Object) -> ^Game_Sequence {
	seq := new(Game_Sequence)
...
deserialize_unit_type :: proc(obj: json.Object) -> ^Unit_Type {
	ut := new(Unit_Type)
...
deserialize_unit_attachment :: proc(obj: json.Object) -> ^Unit_Attachment {
	ua := new(Unit_Attachment)
	unit_attachment_apply_java_defaults(ua)
...
deserialize_player :: proc(obj: json.Object, gd: ^Game_Data) -> ^Game_Player {
	p := new(Game_Player)
	...
	if res_obj, ok := get_object(obj, "resources"); ok {
		p.resources = new(Resource_Collection)
		p.resources.game_data_component.game_data = gd
		p.resources.resources = make(Integer_Map_Resource)
...
deserialize_tech_attachment :: proc(obj: json.Object) -> ^Tech_Attachment {
...
apply_xml_territory_attachments :: proc(gd: ^Game_Data) {
	...
	for terr_name, val in root {
		...
		if t.attachments == nil {
			t.attachments = make(map[string]^I_Attachment)
		}
		t.attachments["territoryAttachment"] = cast(^I_Attachment)ta
		applied += 1
	}
	_ = applied
}
...
deserialize_territory :: proc(obj: json.Object, gd: ^Game_Data) -> ^Territory {
	t := new(Territory)
	...
	t.unit_collection = new(Unit_Collection)
	...
	t.unit_collection.holder = cast(^Named_Unit_Holder)t
...
deserialize_unit :: proc(obj: json.Object, gd: ^Game_Data) -> ^Unit {
	u := new(Unit)
...
json_to_property_value :: proc(val: json.Value) -> Property_Value {
	#partial switch v in val {
	case json.Boolean:
		return v
	case json.Integer:
...
string_to_uuid :: proc(s: string) -> Uuid {
	uuid: Uuid
