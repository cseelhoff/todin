...
// Reads the optional `battleTracker.conquered` array (territory names) from a
// snapshot game-state JSON file. Returns the names so the harness can replay
// them into the freshly-registered battle delegate's tracker after the
// GameData is loaded and delegates are wired (the tracker does not exist at
// JSON-load time). Returns an empty array when the field is absent (older
// snapshots) or on any read/parse error.
load_conquered_territory_names :: proc(snapshot_dir: string, id: string, filename: string) -> [dynamic]string {
	out: [dynamic]string
	full_path := fmt.tprintf("%s/%s/%s", snapshot_dir, id, filename)
	data, read_err := os.read_entire_file(full_path, context.allocator)
	if read_err != nil {
	...
	bt_obj, bt_ok := get_object(root, "battleTracker")
	if !bt_ok {
		return out
	}
	if conq_arr, ok := get_array(bt_obj, "conquered"); ok {
		for item in conq_arr {
...
// Snapshot meta-file RNG state — captured by Java's SnapshotHarness in
// `before-meta.txt` (lines `mt_state: <hex>` and `math_random_seed: <hex>`).
// Both fields are optional: if the snapshot was generated by an older
// harness without these lines, the Odin runner falls back to seeding both
// RNGs to 42 on snap entry (which only matches Java for snaps that
// don't depend on accumulated RNG state from earlier steps).
Snapshot_Rng_State :: struct {
	mt_bytes:        []u8, // nil if not present; 2500 bytes (mti||mt[0..624]) if present
	math_seed:       i64,  // raw 48-bit Math.random LCG state (post-XOR-mask)
	math_seed_present: bool,
}

// Reads `before-meta.txt` (or any meta file) and parses out the optional
// `mt_state` and `math_random_seed` fields. Allocates `mt_bytes` from
// `context.allocator` so the caller is responsible for freeing it.
load_snapshot_rng_state :: proc(snapshot_dir, id, meta_filename: string) -> Snapshot_Rng_State {
	out: Snapshot_Rng_State
	full_path := fmt.tprintf("%s/%s/%s", snapshot_dir, id, meta_filename)
	data, read_err := os.read_entire_file(full_path, context.allocator)
	if read_err != nil {
		return out
	}
	...
	for line in strings.split_lines_iterator(&text) {
		if strings.has_prefix(line, "mt_state: ") {
			hex := strings.trim_space(line[len("mt_state: "):])
			if hex == "" || hex == "unknown" || strings.has_prefix(hex, "error:") {
				continue
			}
			if len(hex) % 2 != 0 {
				log.warnf("Snap %s: mt_state has odd hex length %d, skipping", id, len(hex))
				continue
			}
			bytes := make([]u8, len(hex) / 2)
			ok_decode := true
			for i := 0; i < len(bytes); i += 1 {
				hi := _hex_nibble(hex[2 * i])
				lo := _hex_nibble(hex[2 * i + 1])
				if hi < 0 || lo < 0 {
					ok_decode = false
					break
				}
			...
			if ok_decode {
				...
				delete(bytes)
				log.warnf("Snap %s: mt_state has bad hex, skipping", id)
			}
		} else if strings.has_prefix(line, "math_random_seed: ") {
			hex := strings.trim_space(line[len("math_random_seed: "):])
			if hex == "" || hex == "unknown" || strings.has_prefix(hex, "error:") {
				continue
			}
			val, parse_ok := strconv.parse_u64_of_base(hex, 16)
			if parse_ok {
				out.math_seed = i64(transmute(i64)val)
				out.math_seed_present = true
			} else {
				log.warnf("Snap %s: math_random_seed has bad hex %q, skipping", id, hex)
			}
		}
	}
	return out
